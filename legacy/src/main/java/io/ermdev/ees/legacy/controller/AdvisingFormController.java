...
public class AdvisingFormController implements Initializable {
    ...
    private void initTable() {
        for (io.ermdev.ees.legacy.model.Subject s : subjectDao.getSubjectList())
            SUBJECT_LIST.add(new io.ermdev.ees.legacy.model.recursive.Subject(s.getId(), s.getName(), s.getDesc(), s.getUnit()));
    ...
    public void listener(Student student, List<io.ermdev.ees.legacy.model.recursive.Subject> subjectList) {
        ...
        int tUnit = 0;
        for (io.ermdev.ees.legacy.model.recursive.Subject s : subjectList)
            tUnit += s.getUnit();
