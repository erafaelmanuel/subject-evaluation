...
import io.ermdev.ees.legacy.dao.DirtyDao;
...
import io.ermdev.ees.legacy.stage.SubjectInputStage;
...
public class SubjectInputController implements Initializable, SubjectListStage.OnAddItemListener {
    ...
    @FXML
    private JFXTreeTableView<io.ermdev.ees.legacy.model.recursive.Subject> tblSubject;
    ...
    private final io.ermdev.ees.legacy.model.Subject SUBJECT = new io.ermdev.ees.legacy.model.Subject();
    ...
    protected void onClickSave(ActionEvent event) {
        if (isValid()) {
            ...
            for (io.ermdev.ees.legacy.model.recursive.Subject s : SUBJECT_LIST) {
    ...
    @Override
    public void onAdd(io.ermdev.ees.legacy.model.Subject item) {
        if (subjectDao.getSubjectById(SUBJECT.getId()) != null) {
        ...
        for (io.ermdev.ees.legacy.model.recursive.Subject subject : SUBJECT_LIST) {
            if (subject.getId() == item.getId()) {
                new Thread(() ->
                        JOptionPane.showMessageDialog(null,
        ...
        SUBJECT_LIST.add(new io.ermdev.ees.legacy.model.recursive.Subject(item.getId(), item.getName(), item.getDesc(), item.getUnit()));
    ...
    public void listen(io.ermdev.ees.legacy.model.Subject subject) {
        ...
        SUBJECT_LIST.clear();
        for (io.ermdev.ees.legacy.model.Subject item : new DirtyDaoImpl().getPrerequisiteBySujectId(SUBJECT.getId())) {
            SUBJECT_LIST.add(new io.ermdev.ees.legacy.model.recursive.Subject(item.getId(), item.getName(), item.getDesc(), item.getUnit()));
        }
