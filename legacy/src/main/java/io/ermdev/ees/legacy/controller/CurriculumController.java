...
import io.ermdev.ees.legacy.stage.CurriculumStage;
...
public class CurriculumController implements Initializable, SubjectListStage.OnAddItemListener,
    ...
    @FXML
    private JFXTreeTableView<io.ermdev.ees.legacy.model.recursive.Subject> tblSList;
    ...
    protected void onClickAdd() {
        if (sCurrent != null) {
            ...
            if (!isExist) {
                ...
                SUBJECT_LIST.add(new io.ermdev.ees.legacy.model.recursive.Subject(sCurrent.getId(), sCurrent.getName(),
    ...
    protected void onClickRemove() {
        ...
        if (index >= 0) {
            io.ermdev.ees.legacy.model.recursive.Subject subject =
    ...
    protected void onChooseYS(ActionEvent event) {
        if (cbYS.getSelectionModel().getSelectedIndex() > -1) {
            final int index = cbYS.getSelectionModel().getSelectedIndex();
            final long curriculumId = CURRICULUM_LIST.get(index).getId();
            final List<io.ermdev.ees.legacy.model.recursive.Subject> subjectList = new ArrayList<>();

            totalUnit = 0;

            for (Subject subject : curriculumDao.getSubjectList(curriculumId)) {
                subjectList.add(new io.ermdev.ees.legacy.model.recursive.Subject(subject.getId(), subject.getName(),
