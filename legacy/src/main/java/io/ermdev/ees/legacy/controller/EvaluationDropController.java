...
import io.ermdev.ees.legacy.helper.SectionHelper;
import io.ermdev.ees.legacy.model.Course;
import io.ermdev.ees.legacy.model.Section;
...
import io.ermdev.ees.legacy.stage.EvaluationDropStage;
...
public class EvaluationDropController implements Initializable, AdvisingDoc.CreationListener {
    ...
    private final List<io.ermdev.ees.legacy.model.Subject> NOTSET_LIST = new ArrayList<>();
    ...
    private void addYeSubject(io.ermdev.ees.legacy.model.recursive.Subject subject) {
        for (io.ermdev.ees.legacy.model.recursive.Subject s : ENROLL_SUBJECT_LIST) {
    ...
    private void addYeSubjectAll() {
        first:
        for (io.ermdev.ees.legacy.model.recursive.Subject subject : AVAILABLE_SUBJECT_LIST) {
            for (io.ermdev.ees.legacy.model.recursive.Subject s : ENROLL_SUBJECT_LIST) {
                if (s.getId() == subject.getId())
                    continue first;
            }
    ...
    public void listener(Student student) {
        new Thread(() -> {
            ...
            for(Record record : creditSubjectDao.getRecordListByMark(student.getId(), Remark.NOTSET.getCode())) {
                io.ermdev.ees.legacy.model.Subject subject = subjectDao.getSubjectById(record.getSubjectId());
