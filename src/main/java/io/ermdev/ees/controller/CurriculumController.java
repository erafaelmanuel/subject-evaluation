...
import io.ermdev.ees.stage.CurriculumStage;
...
public class CurriculumController implements Initializable, SubjectListStage.OnAddItemListener,
    ...
    @FXML
    private JFXTreeTableView<Subject> tblSList;
    ...
    private io.ermdev.ees.model.Subject sCurrent;
    ...
    protected void onClickSearch() {
        try {
            ...
            final io.ermdev.ees.model.Subject subject = subjectDao.getSubjectById(subjectId);
    ...
    protected void onClickAdd() {
        if (sCurrent != null) {
            ...
            if (!isExist) {
                ...
                SUBJECT_LIST.add(new Subject(sCurrent.getId(), sCurrent.getName(),
    ...
    protected void onChooseYS(ActionEvent event) {
        if (cbYS.getSelectionModel().getSelectedIndex() > -1) {
            ...
            for (io.ermdev.ees.model.Subject subject : curriculumDao.getSubjectList(curriculumId)) {
                subjectList.add(new Subject(subject.getId(), subject.getName(),
            ...
            if (c != null) {
                ...
                for (int year = 1; year <= TEMP_COURSE.getTotalYear(); year++) {
                    for (int sem = 1; sem <= TEMP_COURSE.getTotalSemester(); sem++) {
                        ...
                        if (isExist) {
                            for (io.ermdev.ees.model.Subject subject : curriculumDao.getSubjectList(COURSE.getId(), year, sem)) {
