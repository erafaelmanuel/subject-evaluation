...
import io.ermdev.ees.helper.DateHelper;
import io.ermdev.ees.helper.DbFactory;
import io.ermdev.ees.helper.EvaluationHelper;
...
public class EvaluationAddController implements Initializable, AdvisingDoc.CreationListener {
    ...
    protected void onChooseFilter() {
        ...
        final List<io.ermdev.ees.model.Subject> list = new ArrayList<>();
        final List<io.ermdev.ees.model.Subject> tempList = new ArrayList<>();
        if(index == -1) return;
        ...
        new Thread(() -> {
            if (index == 0) {
                ...
                for(io.ermdev.ees.model.Subject subject : tempList) {
