...
public class SpecialCurriculumController implements Initializable {
    ...
    @FXML
    private JFXTreeTableView<io.ermdev.ees.model.recursive.Subject> tblFrom;

    @FXML
    private JFXTreeTableView<io.ermdev.ees.model.recursive.Subject> tblTo;
    ...
    private final ObservableList<io.ermdev.ees.model.recursive.Subject> SUBJECT_LIST_TO = FXCollections.observableArrayList();
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        if(cbCourse.getItems().size() > 0 && COURSE_LIST.size() > 0) {
            ...
            for (io.ermdev.ees.model.Subject subject : curriculumDao.getSubjectList(c.getId())) {
                SUBJECT_LIST.add(new io.ermdev.ees.model.recursive.Subject(subject.getId(), subject.getName(), subject.getDesc(), subject.getUnit()));
            }
    ...
    private void onClickAdd() {
        ...
        if (index > -1) {
            ...
            for (io.ermdev.ees.model.recursive.Subject s : SUBJECT_LIST_TO) {
            ...
            TreeItem<io.ermdev.ees.model.recursive.Subject> root = new RecursiveTreeItem<>(SUBJECT_LIST_TO, RecursiveTreeObject::getChildren);
            tblTo.setRoot(root);
            tblTo.setShowRoot(false);
        }
    }
    ...
    private void onClickRemove() {
        ...
        if (index > -1) {
            io.ermdev.ees.model.recursive.Subject subject = SUBJECT_LIST_TO.get(index);
